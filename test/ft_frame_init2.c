...
main(int argc, char *argv[])
{
	...
	/* Z->A INIT ACK is delayed on network and appears now. */ 
	if (!test_for_chunk(SCTP_CID_INIT_ACK, TEST_NETWORK0) ) {
		DUMP_CORE;
	}
	printf("\n\n Z->A INIT_ACK! \n\n");

	change_chunk_sequence(TEST_NETWORK0);
	
	/* A sends INIT_ACK to Z. */
	printf("\n\n A->Z INIT_ACK! \n\n");
	
	/* A sends COOKIE_ECHO to Z, but it will be discarded. */
	...
	/* A sends COOKIE ACK and SACK to Z. */
