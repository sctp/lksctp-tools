...
main(int argc, char *argv[])
{
	...
	printk("\n 1->4 ABORT\n\n");
	if (test_step(SCTP_CID_ABORT, netw[0])<= 0 ) {
		DUMP_CORE;
	}
	...
	printk("Test asoc[2] state CLOSED\n");
	if (asoc[2] && !sctp_state(asoc[2], CLOSED)) {
		DUMP_CORE;
	}

	printk("Test asoc[3] state CLOSED\n");
	if (asoc[3] && !sctp_state(asoc[3], CLOSED)) {
		DUMP_CORE;
	}
