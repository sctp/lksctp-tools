 ...
 * Scenario 3. Two INITs are sent and cross. The following INIt-ACKs and 
 * COOKIE-ECHOs also cross.
 *
 ...
 * setting up procedure step by step. See the association is up and
 * messages appear. Then go home.
 * 
...
main(int argc, char *argv[])
{
	...
	/* We expect 2 COOKIE ECHO and DATA.  */
	...
	/* We expect 2 COOKIE ACK be discarded and 2 SACK . */	
	if (test_step(SCTP_CID_COOKIE_ACK, TEST_NETWORK0) <= 0) {
		DUMP_CORE;
	}
	printf("\n\n Z->A COOKIE_ACK should be discard! \n\n");	
	...
	printf("\n\n A->Z COOKIE_ACK should be discard! \n\n");
