 ...
 * $Id: ft_frame_init5.c, 
 * 
 * This is Functional Test 5 for the SCTP kernel reference
 * implementation state machine.
 * 
 * Case Study 1: Initialization Collision
 * Scenario 5. Two INITs are sent and cross. The following INIt-ACKs and 
 * COOKIE-ECHOs also cross. But one COOKIE-ECHO is lost.
 *
 * Set up a link, send message from sk1 to sk2 and from sk2 to sk1  
 * simultanneously. This will cause overlapping INIT chunk. Go through the
 * setting up procedure step by step, but let Z->A COOKIE_ECHO lost. 
...
main(int argc, char *argv[])
{
	...
	/* Now, let ep2 send another message to ep1 to cause INIT collision.
	 */
	...
	/* Walk through the startup sequence.  */
        /* We should have 2 INIT sitting on the Internet. */
	...
	/* Z->A COOKIE ECHO is lost. */
	if (test_step(SCTP_CID_COOKIE_ECHO, TEST_NETWORK0)<= 0) {
		DUMP_CORE;
	}
	printf("\n\n Let Z->A COOKIE_ECHO missing! \n\n");
	
	/* Let COOKIE ECHO missing. */
	test_kill_next_packet(SCTP_CID_COOKIE_ECHO);

	/* Z sends COOKIE ACK and SACK to A. */
